...
export const signin = async (req, res) => {
  ...
  if (!validPassword)
    return res.status(400).json({ message: "Invalid password" }); // jika password tidak valid, maka tampilkan pesan "Invalid password"
...
export const getSession = async (req, res) => {
  if (!req.session.userId)
    return res.status(404).json({ message: "User not found" }); // jika session userId tidak ada, maka tampilkan pesan "User not found"
  const user = await Users.findOne({
    attributes: ["uuid", "name", "email", "role"], // menampilkan data uuid, name, email, dan role
    where: { uuid: req.session.userId },
  }); // mencari user berdasarkan uuid session userId
...
export const signout = async (req, res) => {
  ...
  req.session.destroy((err) => {
    // jika terjadi error saat menghapus session
